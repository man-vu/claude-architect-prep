...
import { getQuestions, getTheoryQuestions, getScenarioLabel, getDomainLabel, getDomainWeight } from "@/content/i18n";
import { SCENARIOS, DOMAINS } from "@/content/scenarios";
...
export function PracticeView() {
  ...
  const allTheoryQuestions = useMemo(() => getTheoryQuestions(locale), [locale]);
  ...
  if (!filter) {
    return (
      <main className="page-enter mx-auto max-w-3xl px-6 py-10">
        <Link href={localeHref(locale, "/")} className="font-mono text-sm text-accent hover:underline">{t.practice.home}</Link>
        <h1 className="mt-3 font-mono text-2xl font-bold tracking-tight">{t.practice.scenarioTitle}</h1>
        <p className="mt-1 text-sm text-ink-soft">{t.practice.scenarioBlurb}</p>
          ...
          {(Object.keys(SCENARIOS) as ScenarioId[]).map((s, idx) => {
            ...
            return (
              <button key={s} disabled={n === 0} style={{ animationDelay: `${idx * 50}ms` }} onClick={() => { setFilter({ kind: "scenario", id: s }); setI(0); setSelected(null); }} className={topicCls}>
                <div className="font-mono text-sm font-semibold">{getScenarioLabel(locale, s)}</div>
                <div className="mt-0.5 font-mono text-xs text-ink-soft">{t.practice.questionsCount(n)}</div>
              </button>
            );
          })}
        </div>
        <h2 className="mt-8 font-mono text-2xl font-bold tracking-tight">{t.practice.theoryTitle}</h2>
        <p className="mt-1 text-sm text-ink-soft">{t.practice.theoryBlurb}</p>
        <div className="mt-3 grid gap-2 sm:grid-cols-2">
          {(Object.keys(DOMAINS) as Domain[]).map((d, idx) => {
            ...
            return (
              <button key={d} disabled={n === 0} style={{ animationDelay: `${idx * 50}ms` }} onClick={() => { setFilter({ kind: "domain", id: d }); setI(0); setSelected(null); }} className={topicCls}>
                <div className="font-mono text-sm font-semibold">{getDomainLabel(locale, d)}</div>
                <div className="mt-0.5 font-mono text-xs text-ink-soft">{t.practice.theoryDrillsCount(n, getDomainWeight(d))}</div>
              </button>
            );
          })}
        </div>
  ...
  return (
    <main className="px-6 py-8">
      <div className="mx-auto mb-4 flex max-w-3xl items-center justify-between pr-40 lg:pr-0">
        <button onClick={() => setFilter(null)} className="font-mono text-sm text-accent hover:underline">{t.practice.changeTopic}</button>
        <span className="caret font-mono text-sm font-bold text-accent">{t.exam.questionCounter(i + 1, poolLength)}</span>
      </div>
      ...
      <div className="mx-auto mt-6 flex max-w-3xl justify-end">
        <button disabled={selected === null || i + 1 >= poolLength}
          ...
          className="arrow-nudge rounded-md bg-ink px-6 py-2.5 font-mono text-sm font-semibold text-paper transition-colors hover:bg-accent disabled:opacity-40">{t.exam.next} <span className="arrow">→</span></button>
      </div>
    </main>
  );
}
